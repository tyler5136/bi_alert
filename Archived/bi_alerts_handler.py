...
LOG_PATH = rf"C:\scripts\logs\log{today}.txt"
...
def get_alert_from_alertlist(session, camera):
    ...
    try:
        ...
        debug_log(f"🐛 Found {len(alerts)} total alerts in the last {alert_search_time} seconds")
...
def export_clip(session, path, offset, msec):
    ...
    # Determine export duration - use alert duration if <= 60 seconds, otherwise use default 60 seconds
...
def extract_alert_jpegs(mp4_path, jpeg_save_dir, camera_name):
    """
    Extract key JPEGs from MP4 based on alert timing logic:
    - 1 JPEG 2 seconds after start
    - 1 JPEG 3 seconds before end
    - 1 JPEG every 5 seconds throughout the alert
    
    Returns list of local file paths
    """
    debug_log(f"🐛 Starting JPEG extraction from: {mp4_path}")
    debug_log(f"🐛 Save directory: {jpeg_save_dir}")
    
    try:
        # Create output directory if it doesn't exist
        os.makedirs(jpeg_save_dir, exist_ok=True)
        debug_log(f"🐛 Created/verified JPEG directory: {jpeg_save_dir}")
        
        # Check if input file exists
        if not os.path.exists(mp4_path):
            debug_log(f"🐛 ❌ MP4 file does not exist: {mp4_path}")
            raise Exception(f"MP4 file not found: {mp4_path}")
        
        # Open video
        cap = cv2.VideoCapture(mp4_path)
        if not cap.isOpened():
            debug_log("🐛 ❌ Could not open MP4 file for JPEG extraction")
            raise Exception(f"Could not open MP4 file: {mp4_path}")
        ...
        debug_log(f"🐛 Video properties: {width}x{height}, {fps}fps, {total_frames} frames")
        debug_log(f"🐛 Duration: {duration_seconds:.1f} seconds")
        ...
        if duration_seconds > 2:
            ...
            debug_log(f"🐛 Adding extraction at 2.0 seconds (start + 2s)")
        
        # 2. JPEG 3 seconds before end
        if duration_seconds > 3:
            before_end_time = duration_seconds - 3.0
            if before_end_time > 0 and before_end_time not in extraction_times:
                extraction_times.append(before_end_time)
                debug_log(f"🐛 Adding extraction at {before_end_time:.1f} seconds (end - 3s)")
        
        # 3. JPEG every 5 seconds throughout
        current_time = 0
        while current_time < duration_seconds:
            if current_time not in extraction_times:
                extraction_times.append(current_time)
                debug_log(f"🐛 Adding extraction at {current_time:.1f} seconds (5s interval)")
            current_time += 5.0
        
        # Sort extraction times
        extraction_times.sort()
        debug_log(f"🐛 Final extraction times: {extraction_times}")
        
        # Extract frames and save as JPEGs
        extracted_files = []
        timestamp_str = datetime.now().strftime('%m%d%y_%H%M%S')
        
        for i, extract_time in enumerate(extraction_times):
            frame_number = int(extract_time * fps)
            ...
            if not ret:
                debug_log(f"🐛 ⚠️ Could not extract frame at {extract_time:.1f}s")
                continue
            
            # Generate filename
            jpeg_filename = f"{camera_name}_{timestamp_str}_frame_{i+1:02d}_{extract_time:.1f}s.jpg"
            jpeg_path = os.path.join(jpeg_save_dir, jpeg_filename)
            
            # Save as JPEG with high quality
            success = cv2.imwrite(jpeg_path, frame, [cv2.IMWRITE_JPEG_QUALITY, 95])
            
            if success:
                file_size = os.path.getsize(jpeg_path)
                debug_log(f"🐛 ✅ Saved JPEG: {jpeg_filename} ({file_size:,} bytes)")
                extracted_files.append(jpeg_path)
            else:
                debug_log(f"🐛 ❌ Failed to save JPEG: {jpeg_filename}")
        
        cap.release()
        debug_log(f"🐛 ✅ Extracted {len(extracted_files)} JPEG files")
        log(f"✅ Extracted {len(extracted_files)} JPEG frames from alert")
        
        return extracted_files
        
    except Exception as e:
        debug_log(f"🐛 ❌ JPEG extraction failed: {e}")
        log(f"❌ JPEG extraction failed: {e}")
        return []

def upload_jpegs_to_minio(jpeg_file_paths, object_prefix="alert_frames"):
    """
    Upload list of JPEG files to MinIO and return list of URLs
    
    Args:
        jpeg_file_paths: List of local file paths
        object_prefix: MinIO object prefix (folder)
    
    Returns:
        List of MinIO URLs
    """
    debug_log(f"🐛 Starting batch JPEG upload to MinIO")
    debug_log(f"🐛 Files to upload: {len(jpeg_file_paths)}")
    debug_log(f"🐛 Object prefix: {object_prefix}")
    ...
    try:
        ...
        # Upload each JPEG
        for i, jpeg_path in enumerate(jpeg_file_paths, 1):
            try:
                if not os.path.exists(jpeg_path):
                    debug_log(f"🐛 ⚠️ Skipping missing file: {jpeg_path}")
                    continue
                ...
                debug_log(f"🐛 Uploading JPEG {i}/{len(jpeg_file_paths)}: {original_filename}")
                ...
                debug_log(f"🐛 ✅ Uploaded: {original_filename} ({file_size:,} bytes)")
                log(f"✅ Uploaded JPEG {i}/{len(jpeg_file_paths)}: {original_filename}")
                
            except Exception as e:
                debug_log(f"🐛 ❌ Failed to upload {os.path.basename(jpeg_path)}: {e}")
                log(f"❌ Failed to upload JPEG: {os.path.basename(jpeg_path)} - {e}")
        
        debug_log(f"🐛 ✅ Batch upload complete: {len(minio_urls)}/{len(jpeg_file_paths)} successful")
        log(f"✅ Uploaded {len(minio_urls)} JPEG files to MinIO")
        
        return minio_urls
        
    except Exception as e:
        debug_log(f"🐛 ❌ MinIO batch upload failed: {e}")
        log(f"❌ JPEG batch upload failed: {e}")
        return []
...
def send_webhook(gif_minio_url, jpeg_minio_urls=None):
    """Send webhook with MinIO URLs - includes JPEG list for AI processing"""
    global camera_arg, timestamp_arg
    
    debug_log("🐛 Starting webhook send with MinIO URLs")
    debug_log(f"🐛 Main GIF MinIO URL: {gif_minio_url}")
    if jpeg_minio_urls:
        debug_log(f"🐛 JPEG URLs count: {len(jpeg_minio_urls)}")
        for i, url in enumerate(jpeg_minio_urls, 1):
            debug_log(f"🐛   JPEG {i}: {url}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Add JPEG URLs if available
            if jpeg_minio_urls and len(jpeg_minio_urls) > 0:
                data["has_jpegs"] = "true"
                data["jpeg_count"] = str(len(jpeg_minio_urls))
                # Send as comma-separated string for n8n to split
                data["jpeg_urls"] = ",".join(jpeg_minio_urls)
            
            debug_log(f"🐛 Webhook data keys: {list(data.keys())}")
            debug_log(f"🐛 JPEG count being sent: {data.get('jpeg_count', 'none')}")
    ...
    try:
        ...
        if used_fallback or alert_clip is None:
            ...
            try:
                ...
                raise Exception(f"Both direct alert access and alertlist fallback failed. Alertlist: {e}")
        ...
        # Always export the final alert clip
        ...
        # Extract JPEG frames for AI processing
        jpeg_save_dir = os.path.join(GIF_SAVE_DIR, "frames")  # Save in subfolder
        debug_log("🐛 Extracting JPEG frames for AI processing...")
        log("📸 Extracting JPEG frames for AI processing...")
        
        jpeg_file_paths = extract_alert_jpegs(exported_mp4_path, jpeg_save_dir, camera_arg)
        ...
        if converted_gif_path:
            ...
            # Upload JPEG frames if any were extracted
            jpeg_minio_urls = []
            if jpeg_file_paths:
                debug_log("🐛 Uploading JPEG frames to MinIO...")
                log("📤 Uploading JPEG frames to MinIO...")
                jpeg_minio_urls = upload_jpegs_to_minio(jpeg_file_paths, "alert_frames")
                
                if jpeg_minio_urls:
                    log(f"✅ {len(jpeg_minio_urls)} JPEG frames uploaded to MinIO")
                    # Log file sizes comparison
                    try:
                        gif_size = os.path.getsize(converted_gif_path)
                        total_jpeg_size = sum(os.path.getsize(path) for path in jpeg_file_paths if os.path.exists(path))
                        log(f"📏 File sizes - GIF: {gif_size:,} bytes, JPEGs total: {total_jpeg_size:,} bytes")
                        log(f"📏 JPEG efficiency: {len(jpeg_file_paths)} frames vs continuous GIF")
                    except Exception as e:
                        debug_log(f"🐛 ⚠️ Could not compare file sizes: {e}")
                else:
                    log("⚠️ No JPEG frames were uploaded successfully")
            else:
                log("⚠️ No JPEG frames were extracted")
            
            # Send webhook with both GIF and JPEG URLs
            debug_log("🐛 Sending webhook with MinIO URLs...")
            send_webhook(gif_minio_url, jpeg_minio_urls)
            
            log("✅ Process completed successfully!")
            log(f"📊 Summary:")
            total_time = datetime.now() - script_start_time
            ...
            if jpeg_file_paths:
                log(f"  ├─ JPEG frames extracted: {len(jpeg_file_paths)} files")
            log(f"  └─ Files uploaded to MinIO and webhook sent")
        else:
